...
const Header = () => {
  ...
  const { cartItems } = useCart(); // ✅ dynamic cart items
    ...
    <header className="fixed top-0 left-0 w-full z-50">
      <div className="flex items-center justify-between px-6 lg:px-12 py-4 bg-[#1f1b16] shadow-lg">
        ...
        <div className="hidden lg:flex items-center gap-4 text-white">
          ...
          <button
            ...
            className="px-4 py-2 rounded-full border border-[#f5c27a] text-white"
          >
            Login
          </button>
          ...
          <button
            ...
            className="px-4 py-2 rounded-full bg-[#f5c27a] text-black"
          >
            Register
          </button>
        </div>
      ...
      {isOpen && (
        <div className="lg:hidden bg-[#231a14] px-6 py-6 space-y-4 text-white">
          ...
          <button
            onClick={() => alert("📞 Call on this number: +91 99999 99999")}
            className="flex items-center gap-2 mt-4 px-4 py-2 rounded-full border border-[#f5c27a] bg-white/10"
          >
            <Phone size={16} /> Call Now
          </button>

          <div className="flex gap-4 mt-4">
            <button onClick={() => navigate("/login")} className="w-full px-4 py-2 rounded-full border border-[#f5c27a]">
              Login
            </button>
            <button onClick={() => navigate("/register")} className="w-full px-4 py-2 rounded-full bg-[#f5c27a] text-black">
              Register
            </button>
          </div>
        </div>
